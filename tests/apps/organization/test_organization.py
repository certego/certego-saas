...
class TestOrganization(CustomTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # create test users
        cls.user_owner_org_1: User = User.objects.get_or_create(username="test_user_owner_org_1")[0]
        cls.user_admin_org_1: User = User.objects.get_or_create(username="test_user_admin_org_1")[0]
        cls.user_admin2_org_1: User = User.objects.get_or_create(username="test_user_admin2_org_1")[0]
        cls.user_common_org_1: User = User.objects.get_or_create(username="test_user_common_org_1")[0]
        cls.user_common2_org_1: User = User.objects.get_or_create(username="test_user_common2_org_1")[0]
        cls.user_owner_org_2: User = User.objects.get_or_create(username="test_user_owner_org_2")[0]
        cls.user_common_org_2: User = User.objects.get_or_create(username="test_user_common_org_2")[0]
        cls.user_no_org: User = User.objects.get_or_create(username="test_user_no_org")[0]
        cls.user_invited: User = User.objects.get_or_create(username="test_user_invited")[0]
    ...
    def setUp(self) -> None:
        ...
        Organization.objects.all().delete()
        # create test orgs
        self.org_1: Organization = Organization.create(name="testorg1", owner=self.user_owner_org_1)
        self.org_2: Organization = Organization.create(name="testorg2", owner=self.user_owner_org_2)
        # add users to the orgs
        Membership.objects.create(user=self.user_admin_org_1, organization=self.org_1, is_admin=True)
        Membership.objects.create(user=self.user_admin2_org_1, organization=self.org_1, is_admin=True)
    ...
    def test_correct_remove_user_from_org(self):
        ...
        # owner remove a user
        self.client.force_authenticate(self.user_owner_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_admin2_org_1.username})
        self.assertEqual(204, response.status_code)
        # admin remove a user
        self.client.force_authenticate(self.user_admin_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_common_org_1.username})
        self.assertEqual(204, response.status_code)
    ...
    def test_error_remove_user_from_org(self):
        ...
        # 1 - common user cannot remove users (owner, admin or other user)
        self.client.force_authenticate(self.user_common_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_owner_org_1.username})
        self.assertEqual(403, response.status_code)
        response = self.client.post(org_remove_member_uri, {"username": self.user_admin_org_1.username})
        self.assertEqual(403, response.status_code)
        response = self.client.post(org_remove_member_uri, {"username": self.user_common2_org_1.username})
        self.assertEqual(403, response.status_code)
        # 2 - admin cannot remove neither the owner nor other admin
        self.client.force_authenticate(self.user_admin_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_owner_org_1.username})
        ...
        # 4 - request with a username not existing
        self.client.force_authenticate(self.user_admin_org_1)
        response = self.client.post(org_remove_member_uri, {"username": "not_existing_user"})
        self.assertEqual(400, response.status_code)
        self.assertIn("No such member.", response.json()["errors"])
        # 5 - request with a valid username, but it's not a member
        self.client.force_authenticate(self.user_admin_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_no_org.username})
        self.assertEqual(400, response.status_code)
        self.assertIn("No such member.", response.json()["errors"])
        # 6 - request with a valid username and member of another org
        self.client.force_authenticate(self.user_admin_org_1)
        response = self.client.post(org_remove_member_uri, {"username": self.user_common_org_2.username})
        self.assertEqual(400, response.status_code)
        self.assertIn("No such member.", response.json()["errors"])
        # 7 - user with no org
        self.client.force_authenticate(self.user_no_org)
    ...
    def test_error_invite(self):
        ...
        # 1 - Cannot invite member
        self.client.force_authenticate(self.user_owner_org_2)
        response = self.client.post(org_invite_uri, {"username": self.user_common_org_2})
        self.assertEqual(400, response.status_code)
        ...
        self.assertEqual(400, response.status_code)
        self.assertEqual({'username': ['This field is required.']}, response.json()["errors"])
        response = self.client.post(org_invite_uri, {"username": ""})
        self.assertEqual(400, response.status_code)
        self.assertEqual({'username': ['This field may not be blank.']}, response.json()["errors"])
        # 4 - Cannot invite not existing username
        self.client.force_authenticate(self.user_owner_org_2)
        ...
        with transaction.atomic():
            self.client.force_authenticate(self.user_owner_org_2)
            response = self.client.post(org_invite_uri, {"username": self.user_invited.username})
            self.assertEqual(400, response.status_code)
            self.assertIn('Invite failed.', response.json()["errors"])
        # 6 - Cannot invite if max member number is reached
        self.client.force_authenticate(self.user_owner_org_1)
        response = self.client.post(org_invite_uri, {"username": self.user_no_org.username})
        self.assertEqual(400, response.status_code)
        self.assertIn(Invitation.MaxMemberException.default_detail, response.json()["errors"])
        # 7 - cannot invite if no member of org
        self.client.force_authenticate(self.user_no_org)
        response = self.client.post(org_invite_uri, {"username": self.user_owner_org_1.username})
        self.assertEqual(404, response.status_code)
